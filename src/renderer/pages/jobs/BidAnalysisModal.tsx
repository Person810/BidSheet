...
export function BidAnalysisModal({ job, sections, lineItems, onClose }: {
  ...
  const handleExport = async () => {
    ...
    lines.push('﻿' + ['Section', 'Alternate', 'Material', 'Labor', 'Equipment', 'Sub',
      'Direct Cost', 'Sell', 'Margin', 'Margin %', 'Labor Hours', 'Equipment Hours', 'Pipe LF'].map(csvEscape).join(','));
    for (const s of analysis.sections) {
      lines.push([
        ...
        s.laborHours.toFixed(1), s.equipmentHours.toFixed(1), s.pipeLF.toFixed(1),
      ].map(csvEscape).join(','));
    }
    lines.push('');
    lines.push(['Pipe Size (in)', 'Total LF', 'Direct Cost', 'Direct $/LF'].map(csvEscape).join(','));
    for (const p of analysis.pipeSizes) {
      lines.push([p.sizeIn, p.totalLF.toFixed(1), p.directCost.toFixed(2), p.directPerLF.toFixed(2)]
        .map(csvEscape).join(','));
    }
    lines.push('');
      ...
      <div className="modal" style={{ maxWidth: 900 }} onClick={(e) => e.stopPropagation()}>
        ...
        {!hasItems ? (
          <p className="text-muted" style={{ fontSize: 13 }}>
            No line items yet. Add line items to see effort, margin, and $/LF here.
          </p>
        ) : (
          <>
            {/* Effort strip */}
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th className="text-right">Direct Cost</th>
                    <th className="text-right">Direct $/LF</th>
                  </tr>
                </thead>
                <tbody>
                  {analysis.pipeSizes.map((p) => (
                    <tr key={p.sizeIn}>
                      <td style={{ fontWeight: 600 }}>{p.sizeIn}"</td>
                      <td className="text-right">{fmt1(p.totalLF)}</td>
                      <td className="text-right">{formatCurrency(p.directCost)}</td>
                      <td className="text-right" style={{ fontWeight: 600 }}>{formatCurrency(p.directPerLF)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <p className="text-muted" style={{ fontSize: 11, marginTop: 8 }}>
              ...
              cover LF line items with an inch-marked size — pipe material and its crew/equipment
