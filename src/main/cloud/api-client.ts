...
export interface OrgMember {
  ...
  has_wrap: number;
  /**
   * HMAC of `pubkey` under the invite token this member redeemed, set by them
   * at redeem. Owner-only; null when they joined from a client that predates
   * the binding (v0.3.3), which means *unverified*, not tampered.
   */
  key_binding?: string | null;
  /** The redeemed invite's token, encrypted under the account DEK. Owner-only. */
  invite_enc_token?: string | null;
  /** The redeemed invite's id — the AAD scope for invite_enc_token. Owner-only. */
...
export interface OrgInvite {
  ...
  opened_count: number;
  created_at: string;
  /** The invite token under the account DEK; lets an owner re-display the code. */
...
export class CloudApiClient {
  ...
  /**
   * Create an invite. The token is minted on this machine and only its hash
   * plus `enc_token` (the token under the account DEK) are sent, so the server
   * never sees the raw token and cannot forge a member's key binding with it.
   */
  ...
  async redeemInvite(body: {
    ...
    pubkey: string;
    wrapped_priv: string;
    /** Proves this pubkey was generated by a holder of the invite token. */
    key_binding: string;
